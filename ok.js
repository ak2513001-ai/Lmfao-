(function(){
 if(!window.geofs||!geofs.api?.viewer){alert("GeoFS not ready");return}
 const v=geofs.api.viewer,gl=v.scene.globe,C=Cesium,S={};
 ...
 function setCam(){const p=C.Cartesian3.fromDegrees(S.lon,S.lat,S.alt);v.camera.setView({destination:p,orientation:{heading:rad(S.hdg),pitch:rad(S.pitch),roll:0}})}
 function spawn(){const a=geofs.aircraft.instance;S.lat=a.llaLocation[0];S.lon=a.llaLocation[1];S.hdg=a.heading||0;S.pitch=0;S.alt=gnd(S.lon,S.lat)+1.6;S._t=performance.now();S._bob=0}
 function loop(){if(!S.on)return;const now=performance.now(),dt=Math.min(.05,(now-S._t)/1000);S._t=now;let f=S.mvY,r=S.mvX,L=Math.hypot(f,r);if(L>0){f/=L;r/=L}const sp=(S.mvRun?7:4)*dt,h=rad(S.hdg),n=f*Math.cos(h)-r*Math.sin(h),e=f*Math.sin(h)+r*Math.cos(h);S.lat+=latM(n*sp);S.lon+=lonM(e*sp,S.lat);const speed=Math.hypot(f,r)*(S.mvRun?2:1);S._bob+=dt*speed*6;const bob=Math.sin(S._bob)*0.03;S.alt=gnd(S.lon,S.lat)+1.6+bob;setCam()}
 if(S.on){v.scene.postRender.removeEventListener(S._lp);S._joy.remove();S._css.remove();(S.hid||[]).forEach(([el,d])=>el.style.display=d||"");let c=v.scene.screenSpaceCameraController;c.enableRotate=c.enableZoom=c.enableTilt=c.enableTranslate=true;S.on=false;note("Walk OFF");return}
 spawn();S.mvX=0;S.mvY=0;S.mvRun=false;S._lp=()=>loop();v.scene.postRender.addEventListener(S._lp);
 S.hid=[];document.querySelectorAll(".joystick,.geofs-joystick,.mobile-controls").forEach(el=>{S.hid.push([el,el.style.display]);el.style.display="none"});
 ...
 document.addEventListener("touchstart",e=>{const t=[...e.touches].find(x=>x.clientX>140);if(!t)return;rot=true;rx=t.clientX;ry=t.clientY;rid=t.identifier},{passive:true});
 document.addEventListener("touchmove",e=>{if(!rot)return;const t=[...e.touches].find(x=>x.identifier===rid);if(!t)return;const dx=t.clientX-rx,dy=t.clientY-ry;S.hdg=(S.hdg+dx*0.6+360)%360;S.pitch=Math.min(Math.max(S.pitch-dy*0.3,-89),89);rx=t.clientX;ry=t.clientY},{passive:true});
 document.addEventListener("touchend",()=>rot=false,{passive:true});
 let c=v.scene.screenSpaceCameraController;c.enableRotate=c.enableZoom=c.enableTilt=c.enableTranslate=false;
 S.on=true;note("Walk ON");
})();
