(function() {
    ...
    spawn(); // <--- respawn near aircraft every time
    
    S.mvX=0;S.mvY=0;S.mvRun=false;S.t=performance.now();S._lp=()=>loop();v.scene.postRender.addEventListener(S._lp);S.hid=[];document.querySelectorAll('.joystick,.geofs-joystick,.mobile-controls').forEach(el=>{S.hid.push([el,el.style.display]);el.style.display="none"});
